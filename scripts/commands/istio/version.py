...
@click.option("--cluster-name",
              help="Cluster name to read the istio version from",
              required=True)
def version(cluster_name):
    os.chdir(DEFAULT_CLUSTERS_DIR + cluster_name)

    configuration = ConfigurationParser(DEFAULT_CLUSTER_CONFIG)
    configuration.validate()

    # Print only the raw value so CI can capture it: $(... istio version ...)
    click.echo(configuration.get_istio_version())
